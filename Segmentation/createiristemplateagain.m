...
function [imagewithcircles,x,y,radius,xp,yp,radiusp] = createiristemplateagain(eyeimage_filename)
...
sigmaOnf=0.5;
%eyeimage_filename='F:\4.2\dip\lab\database\file1.jpg';
...
%figure,subplot(1,3,1);imshow(imagewithnoise2);title('noise segmented');
%subplot(1,3,2);imshow(imagewithcircles);title('iris and pupil circles segmented');
%subplot(1,3,3);imshow(eyeimage);title('original image');
%figure,imshow(imagewithcircles);title('iris and pupil circles segmented');
%binarysegmentimage = im2bw(imagewithcircles);
%savepath='F:\4.2\dip\lab\Segmentation\output\';
% model2output(imagewithcircles,x,y,circleiris(3));
radius=circleiris(3);
radiusp=circlepupil(3);
model2outputagain(imagewithcircles,x,y,radius,xp,yp,radiusp);
%imwrite(imagewithcircles,[savepath,'-segmented.jpg'],'jpg');
%cd(w);
